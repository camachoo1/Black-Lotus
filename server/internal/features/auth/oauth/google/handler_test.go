// internal/features/auth/oauth/google/handler_test.go
package google_test
...
// MockService mocks the Google Service for testing
type MockService struct {
	authenticateFunc func(ctx context.Context, code string, redirectURI string) (*models.User, error)
	getAuthURLFunc   func(redirectURI string, state string) string
}

func (m *MockService) Authenticate(ctx context.Context, code string, redirectURI string) (*models.User, error) {
	if m.authenticateFunc != nil {
		return m.authenticateFunc(ctx, code, redirectURI)
	}
...
// TestGetAuthURL tests the Google auth URL endpoint
func TestGetAuthURL(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			path:           "/api/auth/google/url?returnTo=/dashboard",
			returnTo:       "/dashboard",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create handler
			handler := google.NewHandler(mockService, mockSessionService)
...
// TestHandleCallback tests the Google callback handler
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupMocks: func(mockService *MockService, mockSession *MockSessionService, userID uuid.UUID) {
				mockService.authenticateFunc = func(ctx context.Context, code string, redirectURI string) (*models.User, error) {
					return &models.User{
						ID:    userID,
						Name:  "Test User",
